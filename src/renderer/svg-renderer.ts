...
export function renderChartSvg(chart: ZiWeiChart, options: RenderOptions = {}): string {
  ...
  const parts: string[] = [];
  parts.push(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" role="img" aria-label="Zi Wei Dou Shu chart" font-family="'Noto Serif TC','Songti TC','PMingLiU',serif">`);
